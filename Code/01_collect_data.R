...
# At-Bat Data
at_bat <- select(tbl(my_db, "atbat"),
                 gameday_link, batter, pitcher, num, event,
                 inning_side, inning, batter_name, pitcher_name,
                 date,o)

# Pitch Data
pitch <- select(tbl(my_db, "pitch"),
               des, type, x, y, start_speed, end_speed, sz_top,
               sz_bot, pfx_x, pfx_z, x0, y0, z0, vx0, vy0, vz0, ax, ay, az, break_y,
               break_angle, break_length, pitch_type, spin_dir, spin_rate,
               inning_side, inning, num, count, zone, gameday_link)
